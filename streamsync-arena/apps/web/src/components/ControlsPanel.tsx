...
type StreamPreset = {
  ...
  rules: Array<
    Pick<EffectRule, "keyword" | "effect" | "enabled" | "obsActionType">
  >;
};
...
export function ControlsPanel({
  ...
  const [maxActiveParticipants, setMaxActiveParticipants] = useState(
    state.settings.maxActiveParticipants,
  );
  const [effectRules, setEffectRules] = useState<EffectRule[]>(
    state.effectRules,
  );
    ...
    const builtin = [
      {
        name: "GG Burst",
        keyword: "GG",
        effect: "gg-burst" as const,
        obsActionType: "both" as const,
      },
      {
        name: "Cheer Flash",
        keyword: "8888",
      ...
      },
      {
        ...
        obsActionType: "source-toggle" as const,
      },
    ];
  ...
  useEffect(() => {
    ...
    const custom = effectPresets.filter(
      (preset) => !builtInPresetNames.includes(preset.name),
    );
    window.localStorage.setItem(
      "streamsync.effectPresets",
  ...
  const ruleWarnings = useMemo(() => {
    return effectRules
      .map((rule, index) => {
        ...
        if (
          (actionType === "scene-switch" || actionType === "both") &&
          !rule.obsSceneName
        ) {
          return `ルール${index + 1}: OBSシーンが未入力です`;
        }
        if (
          (actionType === "source-toggle" || actionType === "both") &&
          !rule.obsSourceName
        ) {
  ...
  const onboardingChecks = useMemo(() => {
    const hasKeywords =
      entryKeyword.trim().length > 0 && leaveKeyword.trim().length > 0;
  ...
  function normalizeRulesForSave(rules: EffectRule[]) {
    return rules.map((rule) => {
      ...
      if (actionType === "scene-switch") {
        return {
          ...rule,
          obsSourceName: undefined,
          obsSourceEnabled: undefined,
        };
      }
      return rule;
    });
  ...
  function saveTemplateFromRule(index: number) {
    ...
    const customTemplateCount = effectPresets.filter(
      (preset) => !builtInPresetNames.includes(preset.name),
    ).length;
  ...
  function addRule() {
    if (effectRules.length >= billingPlan.maxEffectRules) {
      setSaveStatus("error");
      setSaveMessage(
        `${billingPlan.tier.toUpperCase()}プランの上限（${billingPlan.maxEffectRules}件）に達しています。`,
      );
      return;
    }
  ...
  function applyStreamPreset(presetId: string) {
    ...
    setSaveStatus("done");
    setSaveMessage(
      `${preset.label}テンプレを適用しました。保存ボタンで確定してください。`,
    );
  }
  ...
  function applyObsPreset(
    index: number,
    preset: NonNullable<EffectRule["obsActionType"]>,
  ) {
  ...
  const refreshOperationLogs = useCallback(async () => {
    try {
      const { data } = await api.get<{ ok: boolean; logs: OperationLog[] }>(
        "/operations",
      );
  ...
  async function importTemplates(file: File | null) {
    ...
    const deduped = parsed.filter(
      (item) => !effectPresets.some((preset) => preset.name === item.name),
    );
    const customCount = effectPresets.filter(
      (preset) => !builtInPresetNames.includes(preset.name),
    ).length;
    const importable =
      billingPlan.tier === "pro"
        ? deduped
        : deduped.slice(0, Math.max(0, 3 - customCount));
    if (billingPlan.tier !== "pro" && importable.length < deduped.length) {
      setSaveStatus("error");
      setSaveMessage(
        "Freeプラン上限のため、一部テンプレは取り込まれませんでした。",
      );
    }
      ...
      <div className="stack">
        <div className="row">
          ...
          <span className={`badge ${billingPlan.tier === "pro" ? "good" : ""}`}>
            Plan: {billingPlan.tier.toUpperCase()}（上限{" "}
            {billingPlan.maxEffectRules} ルール）
          </span>
          <span
            className={`badge ${authMe.role === "viewer" ? "danger" : "good"}`}
          >
            Role: {authMe.role}
          </span>
          <span
            className={`badge ${connectionState === "connected" ? "good" : "danger"}`}
          >
          ...
          <span
            className={`badge ${state.platformErrors.length > 0 ? "danger" : "good"}`}
          >
        ...
        <div>
          ...
          >
            ...
            <div className="row">
              <span
                className={`badge ${onboardingProgress === 100 ? "good" : ""}`}
              >
                進捗 {onboardingProgress}%
              </span>
              <button className="button secondary" onClick={startSetupWizard}>
                クイックセットアップ
              </button>
            </div>
          </div>
          <div className="stack" style={{ gap: 6, marginTop: 6 }}>
            {onboardingChecks.map((check) => (
              <div
                key={check.id}
                className={`badge ${check.done ? "good" : ""}`}
                style={{ display: "block" }}
              >
        ...
        {showSetupWizard && (
          <div className="card" style={{ border: "1px solid var(--border)" }}>
            ...
            {wizardStep === 0 && (
              <div className="stack" style={{ gap: 8 }}>
                ...
                <small style={{ color: "var(--muted)" }}>
                  まずは接続状態を安定させると、その後の保存/反映が確実になります。
                </small>
              </div>
            )}
            {wizardStep === 1 && (
              <div className="stack" style={{ gap: 8 }}>
                ...
                <label>
                  ...
                  <input
                    value={leaveKeyword}
                    onChange={(event) => setLeaveKeyword(event.target.value)}
                  />
                </label>
                <label>
                  同時アクティブ人数
                  <input
                    ...
                    onChange={(event) =>
                      setMaxActiveParticipants(Number(event.target.value))
                    }
                  />
                </label>
              </div>
            ...
            <div className="row" style={{ marginTop: 10 }}>
              <button className="button secondary" onClick={closeSetupWizard}>
                閉じる
              </button>
              <button
        ...
        <div className="row">
          ...
          {runtimeHealth && (
            <span className="badge">
              参加 {runtimeHealth.participants} / アクティブ{" "}
              {runtimeHealth.activeParticipants} / 投票{" "}
              {runtimeHealth.hasActiveVote ? "ON" : "OFF"}
            </span>
          )}
          {healthCheckedAt && (
            <span className="badge">
              更新: {new Date(healthCheckedAt).toLocaleTimeString()}
            </span>
          )}
          {healthStatus === "error" && (
        ...
        <div className="row">
          <button
            ...
            disabled={!canWrite}
            onClick={() =>
              void executeAction("マッチ終了", async () =>
                api.post("/rotation/next-match"),
              )
            }
          >
            マッチ終了
          ...
          <button
            ...
            disabled={!canWrite}
            onClick={() =>
              void executeAction("ローテーション", async () =>
                api.post("/rotation/rotate"),
              )
            }
          >
            今すぐローテ
        ...
        <div>
          ...
          <div>
            <div className="row" style={{ justifyContent: "space-between" }}>
              ...
              <div className="row">
                <button className="button secondary" onClick={exportTemplates}>
                  エクスポート
                </button>
                <button
                  className="button secondary"
                ...
                <input
                  ...
                  style={{ display: "none" }}
                  onChange={(event) =>
                    void importTemplates(event.target.files?.[0] ?? null)
                  }
                />
              </div>
            </div>
            ...
            {effectRules.map((rule, index) => (
              ...
              >
                <label>
                  ...
                  <select
                    onChange={(event) =>
                      applyTemplate(index, event.target.value)
                    }
                    defaultValue=""
                  >
                ...
                <button
                  className="button secondary"
                  onClick={() => saveTemplateFromRule(index)}
                >
                  テンプレ保存
                </button>
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateRule(index, {
                        effect: event.target.value as EffectRule["effect"],
                      })
                    }
                  >
                ...
                <label style={{ minWidth: 180 }}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateRule(index, { obsSceneName: event.target.value })
                    }
                ...
                <label style={{ minWidth: 180 }}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateRule(index, { obsSourceName: event.target.value })
                    }
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateRule(index, {
                        obsActionType: event.target
                          .value as EffectRule["obsActionType"],
                      })
                    }
                  >
                ...
                <div className="row" style={{ gap: 6 }}>
                  <button
                    className="button secondary"
                    onClick={() => applyObsPreset(index, "scene-switch")}
                  >
                    Scene
                  </button>
                  <button
                    className="button secondary"
                    onClick={() => applyObsPreset(index, "source-toggle")}
                  >
                    Source
                  </button>
                  <button
                    className="button secondary"
                    onClick={() => applyObsPreset(index, "both")}
                  >
                    Both
                  </button>
                </div>
                ...
                <label className="row" style={{ marginBottom: 10 }}>
                  <input
                    ...
                    onChange={(event) =>
                      updateRule(index, {
                        obsSourceEnabled: event.target.checked,
                      })
                    }
                    disabled={rule.obsActionType === "scene-switch"}
                  />
                ...
                <button
                  className="button secondary"
                  onClick={() => moveRule(index, 1)}
                  disabled={index === effectRules.length - 1}
                >
                  ↓
                </button>
                <button
        ...
        <div className="row">
          ...
          {saveStatus !== "idle" && (
            <span
              className={`badge ${saveStatus === "error" ? "danger" : "good"}`}
            >
              {saveMessage}
            </span>
          )}
          {actionStatus !== "idle" && (
            <span
              className={`badge ${actionStatus === "error" ? "danger" : "good"}`}
            >
              {actionMessage}
            </span>
          )}
          ...
          <div>
            ...
            {operationLogs.slice(0, 10).map((log, index) => (
              <div
                ...
                style={{ display: "block", marginBottom: 6 }}
              >
                {new Date(log.at).toLocaleTimeString()} [{log.action}]{" "}
                {log.detail}
              </div>
            ))}
          </div>
        ...
        {analyticsSummary && (
          <div>
            ...
            <div className="row">
              <span className="badge">
                待機 {analyticsSummary.participantsQueued}
              </span>
              <span className="badge">
                アクティブ {analyticsSummary.participantsActive}
              </span>
              <span className="badge">
                メッセージ {analyticsSummary.messagesInMemory}
              </span>
              <span className="badge">
                ユニーク視聴者 {analyticsSummary.uniqueChattersInMemory}
              </span>
            </div>
            <div className="row" style={{ marginTop: 6 }}>
              <span className="badge">
                投票総数 {analyticsSummary.votesCast}
              </span>
              <span className="badge">
                エフェクト発火 {analyticsSummary.effectsTriggered}
              </span>
              <span className="badge">
                更新{" "}
                {new Date(analyticsSummary.lastUpdatedAt).toLocaleTimeString()}
              </span>
            </div>
          </div>
        )}
        ...
        {!billingStatus.active && billingPlan.tier === "pro" && (
          <div className="badge danger" style={{ display: "block" }}>
            Proプランの請求状態が inactive
            です。更新系操作は無効化されています。
            {billingStatus.trialEndsAt
              ? `（Trial end: ${new Date(billingStatus.trialEndsAt).toLocaleDateString()}）`
              : ""}
          </div>
        )}
      </div>
